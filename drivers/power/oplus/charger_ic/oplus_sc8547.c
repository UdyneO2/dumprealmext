...
OPLUS_SVOOC_BATT_SYS_CURVES svooc_batt_curves_by_tmprange_soc0_2_50[BATT_SYS_MAX] = {
	...
	[BATT_SYS_CURVE_TEMP160_2_TEMP430] = {
		.batt_sys_curve = {
			{VBATT_4200_MV, VBATT_3000_MA, false,	(7 * 60 * 1000)/VOOC_VOL_EVENT_TIME},	//7min
			{VBATT_4450_MV, VBATT_2500_MA, false,	(13 * 60 * 1000)/VOOC_VOL_EVENT_TIME},	//13min
			{VBATT_4450_MV, VBATT_2250_MA, false,	},	//4600mA
			{VBATT_4450_MV, VBATT_2000_MA, false,	},
			{VBATT_4470_MV, VBATT_1500_MA, false,	},
			{VBATT_4480_MV, VBATT_1000_MA, true,	},
		},
...
#define GET_VBAT_PREDATA_DEFAULT (0x64 << CHGPMP_DATA_H_SHIFT) | 0x02;
...
static int oplus_voocphy_send_mesg(void)
{
	...
	//write txbuff //0x2c
	val = (oplus_vooc_mg->voocphy_tx_buff[0] << CHGPMP_DATA_H_SHIFT) | oplus_vooc_mg->voocphy_tx_buff[1];
...
static void sc8547_update_chg_data(void)
{
	...
	ret = i2c_smbus_read_i2c_block_data(__sc->client, SC8547_REG_13, 8, data_block);
	if (ret < 0) {
		if(oplus_vooc_mg) {
			oplus_vooc_mg->voocphy_iic_err = 1;
		}
	...
	oplus_vooc_mg->cp_vsys = ((data_block[6] << 8) | data_block[7])*125 / 100;
...
BattMngr_err_code_e oplus_vooc_variables_reset(void)
{
	...
	oplus_vooc_mg->err_trans_flg = false;
...
BattMngr_err_code_e oplus_vooc2_handle_get_batt_vol_cmd(void)
{
	...
	if (oplus_vooc_mg->batt_soc > VOOC_SOC_HIGH &&  !oplus_vooc_mg->ask_vol_again) {
		...
		oplus_vooc_write_mesg_mask(TX0_DET_BIT3_MASK,
					&oplus_vooc_mg->voocphy_tx_buff[0], BIT_SLEEP);
...
extern int oplus_mt6360_suspend_charger(void);
extern int oplus_mt6360_disable_charging(void);
...
void oplus_vooc_set_status_and_notify_ap(FASTCHG_STATUS fastchg_notify_status)
{
	...
	switch (fastchg_notify_status) {
		...
		case FAST_NOTIFY_USER_EXIT_FASTCHG:
			...
			//fixed issue of not enter ffc at low probability
		...
		case FAST_NOTIFY_BTB_TEMP_OVER:
			oplus_vooc_mg->fastchg_notify_status = fastchg_notify_status;
			sc_err("FAST_NOTIFY_BTB_TEMP_OVER");
			break;
		default:
...
extern int oplus_get_batt_btbntc_tmp(void);
BOOLEAN oplus_vooc_btb_and_usb_temp_detect(void)
{
	...
	btb_temp = oplus_get_batt_btbntc_tmp();
	usb_temp = 25;
...
bool oplus_voocphy_get_btb_temp_over(void)
{
	if (!oplus_vooc_mg) {
		...
		return oplus_vooc_mg->btb_temp_over;
	}
}
...
void oplus_reset_fastchg_after_usbout(void)
{
	...
	if (oplus_vooc_mg->btb_temp_over) {
		sc_info("btb_temp_over plug out\n");
		oplus_vooc_mg->btb_temp_over = false;
	} else {
	...
	oplus_vooc_mg->fastchg_recovering = false;
	oplus_vooc_mg->err_trans_flg = false;
	oplus_vooc_mg->err_trans_cnt = false;
...
static void sc8547_set_pd_svooc_config(vooc_manager *chip, bool enable)
{
	int ret = 0;
	u8 reg = 0;
	if (!chip) {
	...
	ret = sc8547_read_byte(__sc, SC8547_REG_05, &reg);
	if (ret < 0) {
		sc_err("SC8547_REG_05\n");
		return;
	}
	sc_dbg("pd_svooc config SC8547_REG_05 = %d\n", reg);
}

static bool sc8547_get_pd_svooc_config(vooc_manager *chip)
{
	int ret = 0;
	u8 data = 0;
...
void oplus_voocphy_set_pdsvooc_adapter_config(vooc_manager *chip, bool enable)
{
	if (!chip)
		return;

	sc8547_set_pd_svooc_config(chip, enable);
	return;
}

bool oplus_voocphy_get_pdsvooc_adapter_config(vooc_manager *chip)
{
	if (!chip)
		return false;

	return sc8547_get_pd_svooc_config(chip);
}
...
bool oplus_chg_check_pd_svooc_adapater(void)
{
	...
	pdsvooc = (OPLUS_PDSVOOC_ADATER_ID == oplus_vooc_mg->adapter_model_ver)? true : false;
	sc_dbg("curent adapter %s pdsvooc\n",pdsvooc ? "is" : "is not" );
...
BattMngr_err_code_e vooc_curr_event_handle(unsigned long data)
{
	...
	pd_svooc_status = oplus_voocphy_get_pdsvooc_adapter_config(oplus_vooc_mg);
	if (oplus_chg_check_pd_svooc_adapater()) {
		sc_err("pd_svooc_status = %d, chip->cp_ichg = %d\n", pd_svooc_status, oplus_vooc_mg->cp_ichg);
		if (pd_svooc_status == true && oplus_vooc_mg->cp_ichg > 1000) {
			sc_err("IBUS > 1A set 0x5 to 0x28!\n");
			oplus_voocphy_set_pdsvooc_adapter_config(oplus_vooc_mg, false);
		}
	}
	...
	if (!oplus_vooc_mg->btb_temp_over) {//non btb temp over
		...
		if (chg_temp >= LOW_CURR_FULL_T1 && chg_temp <= LOW_CURR_FULL_T2) {		/*12 - 25C*/
			if ((vbat_temp_cur <= RANGE1_LOW_CURR1 && oplus_vooc_mg->bq27541_vbatt >= RANGE1_FULL_VBAT1)
				...
				|| (vbat_temp_cur <= RANGE1_LOW_CURR5 && oplus_vooc_mg->bq27541_vbatt >= RANGE1_FULL_VBAT5)) {
			...
			if ((vbat_temp_cur <= RANGE2_LOW_CURR1 && oplus_vooc_mg->bq27541_vbatt >= RANGE2_FULL_VBAT1)
				...
				|| (vbat_temp_cur <= RANGE2_LOW_CURR5 && oplus_vooc_mg->bq27541_vbatt >= RANGE2_FULL_VBAT5)) {
				term_curr_over_count++;
				if (term_curr_over_count > 5) {
...
int oplus_voocphy_irq_gpio_init(void)
{
	int rc;
...
static int sc8547_svooc_hw_setting(struct sc8547 *sc)
{
	...
	sc8547_write_byte(__sc, SC8547_REG_05, 0x28);	//IBUS_OCP_UCP:3.6A
	sc8547_write_byte(__sc, SC8547_REG_09, 0x03);	//WD:1000ms
...
static int sc8547_vooc_hw_setting(struct sc8547 *sc)
{
	...
	sc8547_write_byte(__sc, SC8547_REG_09, 0x83);	//WD:5000ms
...
static int sc8547_5v2a_hw_setting(struct sc8547 *sc)
{
	...
	//sc8547_write_byte(__sc, SC8547_REG_05, 0x2c);	//IBUS_OCP_UCP:
	sc8547_write_byte(__sc, SC8547_REG_07, 0x04);
	sc8547_write_byte(__sc, SC8547_REG_09, 0x00);	//WD:
...
static int oplus_voocphy_sw_ctrl2_gpio_init(void)
{
	...
	gpio_direction_output(chip->switch1_gpio, 0);
	...
	if (chip->switch1_gpio < 0) {
		...
		if (gpio_is_valid(chip->switch1_gpio)) {
			...
			if (rc) {
				...
				rc = oplus_voocphy_sw_ctrl2_gpio_init();
				if (rc)
...
int oplus_voocphy_init(void) {
	...
	if (oplus_vooc_mg) {
		...
		oplus_vooc_mg->vbus_adjust_cnt = 0;
...
void opchg_voocphy_dump_reg(void)
{
	if (oplus_vooc_mg && oplus_vooc_mg->vops && oplus_vooc_mg->vops->dump_regs) {
		oplus_vooc_mg->vops->dump_regs();
	} else {
