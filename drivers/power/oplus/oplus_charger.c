		...
		case POWER_SUPPLY_PROP_MMI_CHARGING_ENABLE:
			...
			if (val->intval == 0) {
				if(chip->unwakelock_chg == 1) {
					...
					charger_xlog_printk(CHG_LOG_CRTI, "zhouyi ot allow charging.\n");
...
#ifdef OPLUS_FEATURE_CHG_BASIC
					charger_xlog_printk(CHG_LOG_CRTI, "zhouyi allow charging.\n");
...
extern bool voocphy_get_real_fastchg_allow(void);
		...
		const char __user *buff, size_t count, loff_t *ppos)
{
	...
	if (strncmp(proc_chg_cycle_data, "en808", 5) == 0) {
		...
		if (oplus_chg_get_voocphy_support()) {
			if (voocphy_get_real_fastchg_allow()) {
				g_charger_chip->voocphy.fastchg_to_normal = false;
				oplus_chg_clear_chargerid_info();
			}
		}
...
void opchg_ui_soc_decimal_reset(void)
{
	struct oplus_chg_chip *chip = g_charger_chip;
	if (chip == NULL) {
		return;
...
int oplus_chg_parse_charger_dt(struct oplus_chg_chip *chip)
{
	...
	rc = of_property_read_u32(node, "qcom,qc_temp_little_too_hot_decidegc",
			&chip->limits.qc_temp_little_too_hot_decidegc);
	if (rc < 0) {
		chip->limits.qc_temp_little_too_hot_decidegc = 420;
	}
	...
	#ifdef OPLUS_FEATURE_CHG_BASIC
	rc = of_property_read_u32(node, "qcom,qc_temp_warm_fastchg_ma_hight",
			&chip->limits.qc_temp_warm_fastchg_ma_hight);
	if (rc) {
		chip->limits.qc_temp_warm_fastchg_ma_hight =
			chip->limits.qc_temp_warm_fastchg_ma;
	}
	rc = of_property_read_u32(node, "qcom,qc_temp_warm_fastchg_ma_low",
			&chip->limits.qc_temp_warm_fastchg_ma_low);
	if (rc) {
		chip->limits.qc_temp_warm_fastchg_ma_low =
			chip->limits.qc_temp_warm_fastchg_ma;
	}
	...
	#ifdef OPLUS_FEATURE_CHG_BASIC
	rc = of_property_read_u32(node, "qcom,qc_temp_little_hot_fastchg_ma_hight",
			&chip->limits.qc_temp_little_hot_fastchg_ma_hight);
	if (rc) {
		chip->limits.qc_temp_little_hot_fastchg_ma_hight =
			chip->limits.qc_temp_little_hot_fastchg_ma;
	}
	rc = of_property_read_u32(node, "qcom,qc_temp_little_hot_fastchg_ma_low",
			&chip->limits.qc_temp_little_hot_fastchg_ma_low);
	if (rc) {
		chip->limits.qc_temp_little_hot_fastchg_ma_low =
			chip->limits.qc_temp_little_hot_fastchg_ma;
	}
	#endif
	rc = of_property_read_u32(node, "qcom,qc_temp_little_too_hot_fastchg_ma",
			&chip->limits.qc_temp_little_too_hot_fastchg_ma);
	if (rc) {
		chip->limits.qc_temp_little_too_hot_fastchg_ma = 2000;
	}
	...
	chg_err(
			...
			qc_temp_warm_fastchg_ma_hight = %d, \
			qc_temp_warm_fastchg_ma_low = %d, \
			qc_temp_little_hot_fastchg_ma = %d, \
			qc_temp_little_hot_fastchg_ma_hight = %d, \
			qc_temp_little_hot_fastchg_ma_low = %d, \
			qc_temp_little_too_hot_fastchg_ma = %d, \
	...
	chip->qg_ext_sns = of_property_read_bool(node, "qcom,qg-ext-sns");
	...
	chip->soda_rum_support = of_property_read_bool(node, "qcom,soda_rum_support");
	chip->qc_v3p5_spec_support = of_property_read_bool(node, "qcom,qc_v3p5_spec_support");
	chip->qc_chg_tbat_feature = of_property_read_bool(node, "qcom,qc_chg_tbat_feature");
	chip->rum_chg_thermal_feature = of_property_read_bool(node, "qcom,rum_chg_thermal_feature");
	chip->qc_lower_fcc_noshake_support = of_property_read_bool(node, "qcom,qc_lower_fcc_noshake_support");
	chip->limits.qc_temp_little_hot_5v_support = of_property_read_bool(node, "qcom,qc_temp_little_hot_5v_support");
	...
	charger_xlog_printk(CHG_LOG_CRTI,
			...
			temp_little_cool_fastchg_current_ma_high = %d, \
			temp_little_cool_fastchg_current_ma_low = %d, \
...
static void oplus_chg_set_charging_current(struct oplus_chg_chip *chip)
{
	...
	if ((chip->chg_ctrl_by_lcd) && (chip->led_on) &&
				(charging_current > chip->limits.qc_temp_hot_fastchg_ma_led_on)) {
		if (chip->etempstatus == QC_TEMP_STATUS__TOO_HOT) {
			charging_current = chip->limits.qc_temp_hot_fastchg_ma_led_on;
		}
		charger_xlog_printk(CHG_LOG_CRTI,
				"[BATTERY]LED ON, qc_temp_hot_fastchg_ma_led_on: %d\n", charging_current);
	}
...
void oplus_chg_set_input_current_limit(struct oplus_chg_chip *chip)
{
...
#endif
#endif
	if (chip->qc_9v2a_support && chip->chg_ops->get_charger_subtype() == CHARGER_SUBTYPE_QC && chip->cool_down_force_5v == false) {
	...
	if ((chip->chg_ctrl_by_lcd) && (chip->led_on)  && !chip->cool_down && !chip->cool_down_9v2a) {
		...
		charger_xlog_printk(CHG_LOG_CRTI, "[BATTERY]LED STATUS CHANGED, led_temp_status=%d,IS ON current=%d\n", chip->led_temp_status, current_limit);
		}
...
#ifdef OPLUS_FEATURE_CHG_BASIC
	/*zoutao@ODM_HQ.Charge force 5v when soc > cv_soc 2020/08/04*/
	if (chip->soc > chip->cv_soc) {
		vbus_limit = VBUS_LIMIT_5V;
		charger_xlog_printk(CHG_LOG_CRTI, "[BATTERY]soc[%d] > cv_soc[%d]\n", chip->soc, chip->cv_soc);
	}
#endif
	...
	if (chip->qc_9v2a_support && chip->chg_ops->get_charger_subtype() == CHARGER_SUBTYPE_QC) {
		...
		if (vbus_limit == VBUS_LIMIT_5V)
			chip->force_vbus_limit = true;
		else
			chip->force_vbus_limit = false;
		#ifdef OPLUS_FEATURE_CHG_BASIC
		/*zoutao@ODM_HQ.Charge add flash led status 2020/07/23*/
		if (chip->flash_led_status) {
			vbus_limit = VBUS_LIMIT_5V;
			charger_xlog_printk(CHG_LOG_CRTI, "[BATTERY]flash led STATUS CHANGED, IS ON\n");
		}
		#endif
					...
					OPLUS_CHG_STOP_VOTER voter)
{
	...
	switch (voter) {
		...
		case CHG_STOP_VOTER__MAX_CHGING_TIME:
			chip->charging_state = CHARGING_STATUS_CCCV;
#ifdef OPLUS_FEATURE_CHG_BASIC
/*zoutao@ODM_HQ.Charge enable charge when charging start 2020/06/20*/
			if (oplus_vooc_get_allow_reading() == true) {
				chip->chg_ops->charger_unsuspend();
				chip->chg_ops->charging_enable();
			}
...
static bool swjeita_noshake_lower_fcc(struct oplus_chg_chip *chip)
{
	int batt_temp = chip->temperature;
	int future_fcc, now_swjeita_fcc;
	OPLUS_CHG_TBATT_STATUS tbatt_status = chip->tbatt_status;

	if (batt_temp > chip->anti_shake_bound.hot_bound) {					/*53C*/
		tbatt_status = BATTERY_STATUS__HIGH_TEMP;
	} else if (batt_temp >= chip->anti_shake_bound.warm_bound) {			/*45C*/
		tbatt_status = BATTERY_STATUS__WARM_TEMP;
	} else if (batt_temp >= chip->anti_shake_bound.normal_bound) {			/*16C*/
		tbatt_status = BATTERY_STATUS__NORMAL;
	} else if (batt_temp >= chip->anti_shake_bound.little_cool_bound) {	/*12C*/
		tbatt_status = BATTERY_STATUS__LITTLE_COOL_TEMP;
	} else if (batt_temp >= chip->anti_shake_bound.cool_bound) {			/*5C*/
		tbatt_status = BATTERY_STATUS__COOL_TEMP;
	} else if (batt_temp >= chip->anti_shake_bound.little_cold_bound) {	/*0C*/
		tbatt_status = BATTERY_STATUS__LITTLE_COLD_TEMP;
	} else if (batt_temp >= chip->anti_shake_bound.cold_bound) {			/*-3C*/
	...
	case BATTERY_STATUS__HIGH_TEMP:
		return false;
	case BATTERY_STATUS__COLD_TEMP:
		future_fcc = chip->limits.temp_cold_fastchg_current_ma;
		break;
	case BATTERY_STATUS__LITTLE_COLD_TEMP:
		if (vbatt_higherthan_4180mv) {
			future_fcc = chip->limits.temp_little_cold_fastchg_current_ma_low;
		} else {
			future_fcc = chip->limits.temp_little_cold_fastchg_current_ma_high;
		}
		break;
	case BATTERY_STATUS__COOL_TEMP:
		if (vbatt_higherthan_4180mv) {
			future_fcc = chip->limits.temp_cool_fastchg_current_ma_low;
		} else {
			future_fcc = chip->limits.temp_cool_fastchg_current_ma_high;
		}
		break;
	case BATTERY_STATUS__LITTLE_COOL_TEMP:
		if (vbatt_higherthan_4180mv) {
			future_fcc = chip->limits.temp_little_cool_fastchg_current_ma_low;
		} else {
			future_fcc = chip->limits.temp_little_cool_fastchg_current_ma_high;
		}
		break;
	case BATTERY_STATUS__NORMAL:
		future_fcc = chip->limits.temp_normal_fastchg_current_ma;
		break;
	case BATTERY_STATUS__WARM_TEMP:
		future_fcc = chip->limits.temp_warm_fastchg_current_ma;
		break;
	default:
			break;
	}
	now_swjeita_fcc = chip->chg_ops->get_charger_current();
	pr_err("%s: future_fcc=%d, now_swjeita_fcc=%d\n", __func__, future_fcc, now_swjeita_fcc);
	if(future_fcc < now_swjeita_fcc)
		return true;
	else
		return false;
}
...
static bool oplus_chg_check_tbatt_is_good(struct oplus_chg_chip *chip)
{
	...
	if (batt_temp > chip->limits.hot_bat_decidegc || batt_temp < chip->limits.cold_bat_decidegc) {
		...
		ret = true;
#ifdef OPLUS_FEATURE_CHG_BASIC
		if (chip->qc_lower_fcc_noshake_support) {
			if (swjeita_noshake_lower_fcc(chip)) {
				chip->limits.cold_bat_decidegc = chip->anti_shake_bound.cold_bound;
				chip->limits.little_cold_bat_decidegc = chip->anti_shake_bound.little_cold_bound;
				chip->limits.cool_bat_decidegc = chip->anti_shake_bound.cool_bound;
				chip->limits.little_cool_bat_decidegc = chip->anti_shake_bound.little_cool_bound;
...
static bool oplus_chg_check_vchg_is_good(struct oplus_chg_chip *chip)
{
	...
	if (chg_volt > chip->limits.charger_hv_thr) {
		...
		} else  if (chg_volt < 7600 && chg_volt > 5800 && chip->qg_ext_sns) {
			vchg_counts = 0;
			ret = false;
			vchg_status = CHARGER_STATUS__INVALID;
...
extern void oplus_notify_voocphy(int event);
void oplus_chg_set_flash_led_status(bool val)
{
	...
	g_charger_chip->flash_led_status = val;
	if (oplus_chg_get_voocphy_support()) {
		oplus_notify_voocphy(g_charger_chip->flash_led_status);
	} else {
		if(g_charger_chip->qc_9v2a_support &&
		   g_charger_chip->chg_ops->get_charger_subtype() == CHARGER_SUBTYPE_QC) {
			if (g_charger_chip->flash_led_status == true) {
				g_charger_chip->chg_ops->vbus_voltage_write(VBUS_LIMIT_5V);
			} else {
...
static void oplus_chg_check_vooc_temp_status(struct oplus_chg_chip *chip)
{
	...
	if (batt_temp > chip->limits.vooc_high_bat_decidegc) {			/*>45C*/
		if (oplus_vooc_get_fastchg_started() == true) {
				...
				//oplus_chg_set_chargerid_switch_val(0);	//lkl porting
...
static void oplus_chg_variables_init(struct oplus_chg_chip *chip)
{
	...
	chip->user_cool_down= 0;
	chip->cool_down_done = false;
	chip->cool_down_9v2a = false;
	chip->cool_down_force_5v = false;
	chip->tbatt_pre_shake = TBATT_PRE_SHAKE_INVALID;
	chip->tbatt_normal_pre_shake = TBATT_PRE_SHAKE_INVALID;
	chip->led_on = true;
...
static void oplus_chg_get_chargerid_voltage(struct oplus_chg_chip *chip)
{
	...
	if (oplus_vooc_get_vooc_switch_val() == 1) {
		if (chip->chargerid_volt_got == false) {
			charger_xlog_printk(CHG_LOG_CRTI, "route 1\n");
			...
			if (chip->chg_ops->get_chargerid_switch_val() == 0) {
				charger_xlog_printk(CHG_LOG_CRTI, "route 2\n");
				chip->chg_ops->set_chargerid_switch_val(1);
			} else {
				/* do nothing*/
			}
		...
		if (chip->chargerid_volt_got == false) {
			charger_xlog_printk(CHG_LOG_CRTI, "route 3\n");
			chip->chg_ops->set_chargerid_switch_val(1);
			...
			charger_xlog_printk(CHG_LOG_CRTI, "1route 4\n");
...
static void oplus_chg_qc_config(struct oplus_chg_chip *chip);
extern void opchg_voocphy_dump_reg(void);
static void oplus_chg_fast_switch_check(struct oplus_chg_chip *chip)
{
	...
	charger_xlog_printk(CHG_LOG_CRTI, "fastchg_started %d reset_mcu_delay = %d\n", oplus_vooc_get_fastchg_started(), reset_mcu_delay);
	if (chip->charger_type == POWER_SUPPLY_TYPE_USB_DCP) {
		...
		if (oplus_vooc_get_fastchg_started() == false
			...
			charger_xlog_printk(CHG_LOG_CRTI, "oplus_vooc_switch_fast_chg finished charger_type = %d\n", chip->charger_type);
			if (oplus_chg_get_voocphy_support()
				&& (high_vol_light == true ||
				(oplus_vooc_get_fastchg_dummy_started() == true && voocphy_get_real_fastchg_allow()))) {
				oplus_vooc_switch_mode(NORMAL_CHARGER_MODE);
				oplus_chg_suspend_charger();
				msleep(1500);
				oplus_chg_unsuspend_charger();
				chg_debug("%s reset\n", high_vol_light ? "high_vol_light" : "recover real fastchg");
				if (high_vol_light) {
					high_vol_light = false;
				}
		...
		if (!oplus_vooc_get_fastchg_started()
			...
			if (oplus_chg_get_voocphy_support() && reset_mcu_delay == RESET_MCU_DELAY_30S) {
				cpuboost_charge_event(CPU_CHG_FREQ_STAT_AUTO);
				oplus_basetimer_monitor_stop();
				charger_xlog_printk(CHG_LOG_CRTI, "30s voocphy timeout adjust cpu freq to auto\n");
			}
		...
		if (reset_mcu_delay >= RESET_MCU_DELAY_30S) {
			chip->pd_svooc = false;
			if (oplus_chg_get_voocphy_support() && chip->vooc_start_fail == false) {
...
static bool oplus_chg_check_ffc_status(struct oplus_chg_chip *chip)
{
	...
	charger_xlog_printk(CHG_LOG_CRTI,"route 1: fastchg_to_ffc[%d] fastchg_ffc_status[%d] ffc_temp_status[%d] icharging[%d] batt_volt[%d]",
		chip->fastchg_to_ffc, chip->fastchg_ffc_status, chip->ffc_temp_status, chip->icharging, chip->batt_volt);
		...
		if (chip->fastchg_ffc_status == 2) {
			...
			if (chip->batt_volt >= ffc_vfloat_sw_limit) {
				...
				if (vffc2_counts >= FFC_VOLT_COUNTS) {
					oplus_chg_ffc_variable_reset(chip);
					oplus_chg_turn_on_charging(chip);
					charger_xlog_printk(CHG_LOG_CRTI,"route 4");
					return true;
				}
			}
			if (chip->ffc_temp_status == FFC_TEMP_STATUS__NORMAL) {
				if ((chip->icharging * -1) < (chip->limits.ffc2_normal_fastchg_ma
					...
					if (normal_counts >= FFC_CURRENT_COUNTS) {
						oplus_chg_ffc_variable_reset(chip);
						oplus_chg_turn_on_charging(chip);
						charger_xlog_printk(CHG_LOG_CRTI,"route 5");
						return true;
					}
				} else {
					normal_counts = 0;
				...
				if ((chip->icharging * -1) < (chip->limits.ffc2_warm_fastchg_ma
					...
					if (warm_counts >= FFC_CURRENT_COUNTS) {
						oplus_chg_ffc_variable_reset(chip);
						oplus_chg_turn_on_charging(chip);
						charger_xlog_printk(CHG_LOG_CRTI,"route 6");
						return true;
					}
				} else {
					warm_counts = 0;
				...
				oplus_chg_ffc_variable_reset(chip);
				oplus_chg_turn_on_charging(chip);
				charger_xlog_printk(CHG_LOG_CRTI,"route 7");
				return true;
			}
		}
		return false;
	...
	warm_counts = 0;
	normal_counts = 0;
	charger_xlog_printk(CHG_LOG_CRTI,"route 8");
	return true;
}
...
static void oplus_chg_check_status_full(struct oplus_chg_chip *chip)
{
	...
	if (oplus_vooc_get_allow_reading() == false) {
		...
		charger_xlog_printk(CHG_LOG_CRTI, "%d %d %d %d %d %d\n",
			oplus_vooc_get_fastchg_to_normal(), fastchg_present_wait_count,
			oplus_vooc_get_fastchg_to_warm(), chip->chg_ops->get_charging_enable(),
			chip->charging_state, chip->ffc_temp_status);
		if (((oplus_vooc_get_fastchg_to_normal()== true)
				|| (oplus_vooc_get_fastchg_to_warm() == true))
...
static void oplus_chg_print_log(struct oplus_chg_chip *chip)
{
	if (chip->external_gauge) {
		...
		charger_xlog_printk(CHG_LOG_CRTI,
			...
			chip->otg_switch, chip->mmi_chg, chip->boot_reason, chip->boot_mode,
			chip->chargerid_volt, chip->chargerid_volt_got, chip->shell_temp,
			chip->limits.vbatt_pdqc_to_5v_thr, chip->qc_v3p5_spec_support,
			chip->qc_chg_tbat_feature, chip->rum_chg_thermal_feature);
	}
...
#ifdef OPLUS_FEATURE_CHG_BASIC
static int g_arryQcTmpStep[QC_TEMP_STATUS__INVALID + 1];
#define QC_TEMP_JEITA 10
#define DOWNCOUNT 3
#endif
static bool oplus_chg_get_qc_tbatt_status(struct oplus_chg_chip *chip)
{
	static int itempdowncount = 1;
	static int ipretemp       = 0;
	static bool isfistcheck   = true;
	static bool isstatushold  = true; /* check if there is a temprature update skip */
	static OPLUS_CHG_QC_TEMP_STATUS epretempstatus = QC_TEMP_STATUS__INVALID;
	int i = 0;
	bool ischangestatus       = false; /* need to change current or not */
	int batt_temp             = chip->temperature;
	chip->etempstatus = QC_TEMP_STATUS__INVALID;

	if (batt_temp > chip->limits.warm_bat_decidegc) { /*45C*/
		chip->etempstatus = QC_TEMP_STATUS__INVALID;
	} else if (batt_temp >= chip->limits.qc_temp_little_too_hot_decidegc) { /*42C*/
		chip->etempstatus = QC_TEMP_STATUS__TOO_HOT;
	} else if (batt_temp >= chip->limits.qc_temp_little_hot_decidegc) { /*40C*/
		chip->etempstatus = QC_TEMP_STATUS__HOT;
	} else if (batt_temp >= chip->limits.qc_temp_warm_decidegc) { /*37C*/
		chip->etempstatus = QC_TEMP_STATUS__LITTLE_HOT;
	} else if (batt_temp >= chip->limits.qc_temp_normal_decidegc) {	/*34C*/
		chip->etempstatus = QC_TEMP_STATUS__WARM;
	} else if (batt_temp >= chip->limits.qc_temp_cool_decidegc + 1) { /*22 + 1C*/
		chip->etempstatus = QC_TEMP_STATUS__NORMAL;
	} else if (batt_temp >= chip->limits.normal_bat_decidegc) {	/*16C*/
		chip->etempstatus = QC_TEMP_STATUS__COOL;
	} else {
		chip->etempstatus = QC_TEMP_STATUS__INVALID;
	}
#ifdef OPLUS_FEATURE_CHG_BASIC
	chg_debug(" chip->etempstatus=%d epretempstatus=%d ipretemp=%d batt_temp=%d\n", chip->etempstatus, epretempstatus, ipretemp, batt_temp);
	if (isfistcheck) {
		ipretemp    = batt_temp;
		isfistcheck = false;
		g_arryQcTmpStep[QC_TEMP_STATUS__VIRTUAL]    = -1000;
		g_arryQcTmpStep[QC_TEMP_STATUS__COOL]       = chip->limits.qc_temp_cool_decidegc;/* 22 */
		g_arryQcTmpStep[QC_TEMP_STATUS__NORMAL]     = chip->limits.qc_temp_normal_decidegc;/* 34 */
		g_arryQcTmpStep[QC_TEMP_STATUS__WARM]       = chip->limits.qc_temp_warm_decidegc;/* 37 */
		g_arryQcTmpStep[QC_TEMP_STATUS__LITTLE_HOT] = chip->limits.qc_temp_little_hot_decidegc;/* 40C */
		g_arryQcTmpStep[QC_TEMP_STATUS__HOT]        = chip->limits.qc_temp_little_too_hot_decidegc;/* 42C */
		g_arryQcTmpStep[QC_TEMP_STATUS__TOO_HOT]        = chip->limits.warm_bat_decidegc;
		for (i = 0; i < QC_TEMP_STATUS__INVALID; i++)
		chg_debug("g_arryQcTmpStep[%d]=%d\n", i, g_arryQcTmpStep[i]);
		return true;
	}

	if (QC_TEMP_STATUS__INVALID == chip->etempstatus) {
		return ischangestatus;
	}

	if (epretempstatus != chip->etempstatus) {
		chg_debug("temp_state chip->etempstatus=%d epretempstatus=%d\n", chip->etempstatus, epretempstatus);
		isstatushold   = true;
		if ((ipretemp < batt_temp) && (batt_temp < g_arryQcTmpStep[chip->etempstatus]) && (batt_temp >= g_arryQcTmpStep[chip->etempstatus-1])) {
			itempdowncount = 0;
			ischangestatus = true;
			chg_debug("ipretemp=%d, batt_temp=%d, g_arryQcTmpStep[chip->etempstatus]=%d, g_arryQcTmpStep[chip->etempstatus-1]=%d\n",
				ipretemp, batt_temp, g_arryQcTmpStep[chip->etempstatus], g_arryQcTmpStep[chip->etempstatus-1]);
		} else if (ipretemp > batt_temp) {
			itempdowncount++;
			if (itempdowncount > DOWNCOUNT) {
				itempdowncount = 0;
				if (chip->qc_chg_tbat_feature) {
					ischangestatus = (((batt_temp < (g_arryQcTmpStep[chip->etempstatus] - QC_TEMP_JEITA)) ||
					((batt_temp < g_arryQcTmpStep[chip->etempstatus]) &&
					(g_arryQcTmpStep[chip->etempstatus] == chip->limits.qc_temp_normal_decidegc)) ||
					((chip->qc_lower_fcc_noshake_support) && (batt_temp <= g_arryQcTmpStep[QC_TEMP_STATUS__COOL]))) &&
					(batt_temp >= (g_arryQcTmpStep[chip->etempstatus-1]))) ? true: false;
				} else {
					ischangestatus = (((batt_temp < (g_arryQcTmpStep[chip->etempstatus] - QC_TEMP_JEITA)) ||
					((chip->qc_lower_fcc_noshake_support) && (batt_temp <= g_arryQcTmpStep[QC_TEMP_STATUS__COOL]))) &&
					(batt_temp >= (g_arryQcTmpStep[chip->etempstatus-1]))) ? true: false;
				}
				chg_debug("***ipretemp:%d,batt_temp%d,[g_arryQcTmpStep[chip->etempstatus] - QC_TEMP_JEITA]%d,[g_arryQcTmpStep[chip->etempstatus - 1]]=%d,ischange=%d***\n",
					ipretemp, batt_temp, g_arryQcTmpStep[chip->etempstatus] - QC_TEMP_JEITA, \
					g_arryQcTmpStep[chip->etempstatus - 1], ischangestatus);
			} else {
				isstatushold = false;
				chg_debug("+++pretem:%d,nowtem:%d,count:%d+++\n", ipretemp, batt_temp, itempdowncount);
			}
		}
	}
	if (isstatushold) {
		ipretemp = batt_temp; /* update temprature for there was a update skiped */
		chg_debug("---pretem:%d,nowtem:%d---\n", ipretemp, batt_temp);
	}

	epretempstatus = ischangestatus ? chip->etempstatus : epretempstatus;
#endif
	return ischangestatus;
}
static void oplus_chg_qc_temp_to_current(struct oplus_chg_chip *chip) {
#ifdef OPLUS_FEATURE_CHG_BASIC
	static int more_than_20s;
	more_than_20s = chip->total_time % 40;

	if (chip->qc_chg_tbat_feature) {
		if (chip->temperature >= chip->limits.qc_temp_normal_decidegc
			&& chip->temperature < chip->limits.qc_temp_little_hot_decidegc) {
			if (chip->temperature >= 340 && chip->temperature < 355) {
				if (more_than_20s <= 15)
					chip->limits.temp_normal_fastchg_current_ma = 3400;
				else
					chip->limits.temp_normal_fastchg_current_ma = 3000;
			} else if (chip->temperature >= 355 && chip->temperature < 360) {
				if (more_than_20s <= 15)
					chip->limits.temp_normal_fastchg_current_ma = 3000;
				else
					chip->limits.temp_normal_fastchg_current_ma = 2800;
			} else if (chip->temperature >= 360 && chip->temperature < 370) {
				if (more_than_20s <= 15)
					chip->limits.temp_normal_fastchg_current_ma = 2600;
				else
					chip->limits.temp_normal_fastchg_current_ma = 2000;
			} else if (chip->temperature >= 370 && chip->temperature < 400) {
					chip->limits.temp_normal_fastchg_current_ma = 2000;
			}
		}
	}
...
#ifdef OPLUS_FEATURE_CHG_BASIC
	if (chip->rum_chg_thermal_feature) {
		if (chip->temperature >= chip->limits.qc_temp_normal_decidegc
			&& chip->temperature < chip->limits.qc_temp_warm_decidegc) { /* 36.5C~37.0C */
			if (chip->temperature >=370 && chip->temperature < 373) {
				if (vbatt_higherthan_4180mv) {
					chip->limits.temp_normal_fastchg_current_ma = 2550;
				} else {
					chip->limits.temp_normal_fastchg_current_ma = 3300;
				}
			} else if (chip->temperature >=373 && chip->temperature < 376) {
				if (vbatt_higherthan_4180mv) {
					chip->limits.temp_normal_fastchg_current_ma = 2550;
				} else {
					chip->limits.temp_normal_fastchg_current_ma = 3100;
				}
			} else if (chip->temperature >=376 && chip->temperature < 379) {
				if (vbatt_higherthan_4180mv) {
					chip->limits.temp_normal_fastchg_current_ma = 2550;
				} else {
					chip->limits.temp_normal_fastchg_current_ma = 2900;
				}
			} else if (chip->temperature >=379 && chip->temperature < 382) {
				if (vbatt_higherthan_4180mv) {
					chip->limits.temp_normal_fastchg_current_ma = 2550;
				} else {
					chip->limits.temp_normal_fastchg_current_ma = 2700;
				}
			} else if (chip->temperature >=382 && chip->temperature < 385) {
					chip->limits.temp_normal_fastchg_current_ma = 2550;
			}
			return;
		}
...
#ifdef OPLUS_FEATURE_CHG_BASIC
	if (!oplus_chg_get_qc_tbatt_status(chip) && !chip->vbat_4180mv_status_changed) {
		return;
	}
#endif
	chg_err(" before chip->temperature = %d, chip->limits.temp_normal_fastchg_current_ma= %d\n",
		chip->temperature, chip->limits.temp_normal_fastchg_current_ma);

#ifdef OPLUS_FEATURE_CHG_BASIC
	if (chip->vbat_4180mv_status_changed)
		chip->vbat_4180mv_status_changed = false;
#endif
	if (chip->temperature >= chip->limits.normal_bat_decidegc/* 16C */
		&& chip->temperature <= chip->limits.qc_temp_cool_decidegc) {/* 22C */
		chip->limits.temp_normal_fastchg_current_ma
			= chip->limits.qc_temp_cool_fastchg_ma;
	} else if (chip->temperature > chip->limits.qc_temp_cool_decidegc/* 22C */
		&& chip->temperature < chip->limits.qc_temp_normal_decidegc) {/* soda:34C rum:36C */
		if (vbatt_higherthan_4180mv) {
			chip->limits.temp_normal_fastchg_current_ma
				= chip->limits.qc_temp_normal_fastchg_ma_low;
		} else {
	...
	} else if (chip->temperature >= chip->limits.qc_temp_normal_decidegc/* soda:34C rum:36C */
		&& chip->temperature < chip->limits.qc_temp_warm_decidegc) {	/* soda:37C rum:37.5C */
		#ifdef OPLUS_FEATURE_CHG_BASIC
		if (vbatt_higherthan_4180mv) {
			chip->limits.temp_normal_fastchg_current_ma
				= chip->limits.qc_temp_warm_fastchg_ma_low;
		...
		chip->limits.temp_normal_fastchg_current_ma
			= chip->limits.qc_temp_warm_fastchg_ma;
		#endif
	} else if (chip->temperature >= chip->limits.qc_temp_warm_decidegc
		&& chip->temperature < chip->limits.qc_temp_little_hot_decidegc) {
		#ifdef OPLUS_FEATURE_CHG_BASIC
		if (vbatt_higherthan_4180mv) {
			chip->limits.temp_normal_fastchg_current_ma
		...
		chip->limits.temp_normal_fastchg_current_ma
			= chip->limits.qc_temp_little_hot_fastchg_ma;
		#endif
	} else if (chip->temperature >= chip->limits.qc_temp_little_hot_decidegc
		&& chip->temperature < chip->limits.qc_temp_little_too_hot_decidegc) {
		chip->limits.temp_normal_fastchg_current_ma
			= chip->limits.qc_temp_little_too_hot_fastchg_ma;
	} else if (chip->temperature >= chip->limits.qc_temp_little_too_hot_decidegc
		&& chip->temperature < chip->limits.warm_bat_decidegc) {
		chip->limits.temp_normal_fastchg_current_ma
			= chip->limits.qc_temp_hot_fastchg_ma;
	}

	chg_err(" after chip->temperature = %d, chip->limits.temp_normal_fastchg_current_ma= %d\n",
		chip->temperature, chip->limits.temp_normal_fastchg_current_ma);
}
static void oplus_chg_qc_config(struct oplus_chg_chip *chip)
{
	...
	if (chip->qc_9v2a_support && chip->chg_ops->get_charger_subtype() == CHARGER_SUBTYPE_QC) {
		chip->chg_ops->chg_qc_temp_to_vchg();
		oplus_chg_qc_temp_to_current(chip);
		oplus_chg_set_charging_current(chip);
	}
}
...
bool oplus_voocphy_get_fastchg_ing(void)
{
	if (!g_charger_chip) {
		...
		return g_charger_chip->voocphy.fastchg_ing;
	}
}
...
bool oplus_voocphy_get_fastchg_start(void)
{
	if (!g_charger_chip) {
		...
		return g_charger_chip->voocphy.fastchg_start;
	}
}
...
bool oplus_voocphy_get_fastchg_to_normal(void)
{
	if (!g_charger_chip) {
		...
		return g_charger_chip->voocphy.fastchg_to_normal;
	}
}
...
bool oplus_voocphy_get_fastchg_to_warm(void)
{
	if (!g_charger_chip) {
		...
		return g_charger_chip->voocphy.fastchg_to_warm;
	}
}
...
bool oplus_voocphy_get_fastchg_dummy_start(void)
{
	if (!g_charger_chip) {
		...
		return g_charger_chip->voocphy.fastchg_dummy_start;
	}
}
...
int oplus_voocphy_get_fast_chg_type(void)
{
	if (!g_charger_chip) {
		...
		return g_charger_chip->voocphy.fast_chg_type;
	}
}

void opchg_voocphy_clear_dbg_info(void)
{
	struct oplus_chg_chip *chip = g_charger_chip;
	if (chip) {
		chip->detect_detach_unexpeactly = 0;
		chip->voocphy.cpu0_freq = 0;
		chip->voocphy.cpu1_freq = 0;
		chip->voocphy.cpu2_freq = 0;
		chip->voocphy.cpu3_freq = 0;
		chip->voocphy.ap_disconn_issue = 0;
		chip->voocphy.disconn_pre_ibat = 0;
		chip->voocphy.disconn_pre_ibus = 0;
		chip->voocphy.disconn_pre_vbat = 0;
		chip->voocphy.disconn_pre_vbat_calc = 0;
		chip->voocphy.disconn_pre_vbus = 0;
		chip->voocphy.r_state = 0;
		chip->voocphy.vbus_adjust_cnt = 0;
		chip->voocphy.voocphy_cp_irq_flag = 0;
		chip->voocphy.voocphy_enable = 0;
		chip->voocphy.voocphy_iic_err = 0;
		chip->voocphy.voocphy_vooc_irq_flag = 0;
		memset(chip->voocphy.reg_dump, 0, DUMP_REG_CNT);
	}
}

void opchg_voocphy_clear_cnt_info(void)
{
	struct oplus_chg_chip *chip = g_charger_chip;
	if (chip) {
		chip->voocphy.irq_rcvok_num = 0;
		chip->voocphy.ap_handle_timeout_num = 0;
		chip->voocphy.rcv_done_200ms_timeout_num = 0;
		chip->voocphy.rcv_date_err_num = 0;
	}
}
...
void oplus_chg_set_usb_mid_boost(bool boost)
{
	if (!g_charger_chip) {
		return;
	} else {
		if (g_charger_chip->chg_ops && g_charger_chip->chg_ops->set_usb_mid_boost)
			g_charger_chip->chg_ops->set_usb_mid_boost(boost);
	}
}
#endif
...
static const int cool_down_current_limit_normal1[6] = {500, 900, 1200, 1500, 2000, 1500};
...
void oplus_smart_charge_by_cool_down(struct oplus_chg_chip *chip, int val)
{
	...
	if (chip->qc_9v2a_support) {
		if (val > 6) {
			chip->limits.input_current_charger_ma = cool_down_current_limit_normal1[6 - 1];
			chip->limits.pd_input_current_charger_ma = cool_down_current_limit_normal1[6 - 1];
			chip->limits.qc_input_current_charger_ma = cool_down_current_limit_normal1[6 - 1];
			...
			chip->limits.input_current_charger_ma = cool_down_current_limit_normal1[val - 1];
			chip->limits.pd_input_current_charger_ma = cool_down_current_limit_normal1[val - 1];
			chip->limits.qc_input_current_charger_ma = cool_down_current_limit_normal1[val - 1];
...
int oplus_chg_match_temp_for_chging(void)
{
	...
	if (oplus_chg_override_by_shell_temp(batt_temp)) {
		...
		if(diff < 150 && diff > -150  && batt_temp >= 320 && batt_temp < g_charger_chip->limits.hot_bat_decidegc) {
...
bool opchg_get_vooc_start_fg(void)
{
	if (!g_charger_chip) {
		return 0;
	} else {
